...
class Bot(QWidget):  # y = 21, x = 63
    ...
    def bot_answer(self, ph):  # Ответы бота
        ph = ph.lower()
        # ————————————————————————————————————————————————————————————————
        if ('прив' in ph) or ('здравствуй' in ph) or ('хай' in ph) or ('хеллоу' in ph) or ('здаров' in ph):
            return choice(['Привет', 'Приветик', 'Здравствуйте', 'Приветствую', 'Здравствуй'])
        # ————————————————————————————————————————————————————————————————
        ...
        # ————————————————————————————————————————————————————————————————
        elif ('как' in ph and 'дела' in ph) or (('чё' in ph or 'че' in ph) and 'как' in ph):
            self.flag_how_are_you = True
            return choice(
                ['Всё в порядке.', 'Просто отлично!', 'Волшебно!', 'Феерично!', 'Всё классно.']) + ' А у вас как?'
        # ————————————————————————————————————————————————————————————————
        elif 'хорошо' in ph or 'нормально' in ph or 'тоже' in ph or 'феерично' in ph or 'волшебно' in ph or 'отлично' \
                in ph or 'классно' in ph or 'прекрасно' in ph or ':)' in ph or 'дельно' in ph or 'аналогично' in ph \
                or 'намана' in ph or 'норм' in ph or 'живу' in ph:
            if self.flag_how_are_you:
                self.flag_how_are_you = False
                return choice(['Я за вас рада!', 'Как же это прекрасно!', 'Вот и чудненько!'])
            else:
                return choice(['Что вы имеете ввиду?', 'Что, что?'])
        # ————————————————————————————————————————————————————————————————
        elif 'плохо' in ph or 'отвратительно' in ph or 'ужасно' in ph or ':(' in ph or 'плоховато' in ph or \
                ('так' in ph and 'себе' in ph):
            return 'Не беспокойтесь, всё будет хорошо.'
        # ————————————————————————————————————————————————————————————————
        elif ('завтра' in ph or 'планируешь' in ph or 'хочешь' in ph or 'думаешь' in ph) or (
                'заняться' in ph or 'занимаешься' in ph or 'заниматься' in ph) and 'чем' in ph:
            return choice(['Мне охото', 'Я думаю', 'Я планирую', 'Я собираюсь', 'Я хочу']) + choice(
                [' отдохнуть на диванчике!', ' ничего не делать...', ' дома сидеть', ' пойти гулять',
                 ' пойти на пробежку', ' приготовить вкусный ужин', ' работать над проектом', ' спать весь день',
                 ' учить PyQT'])
        # ————————————————————————————————————————————————————————————————
        elif (('как' in ph or 'какое' in ph) and ('тебя' in ph or 'ты' in ph) and (
                'зовут' in ph, 'имя' in ph, 'название' in ph)) or ('кто' in ph and 'ты' in ph):
            return choice(
                ['А как вы думаете?', 'Меня зовут Mimino', 'Я Mimino', 'Моё имя Mimino'])
        # ————————————————————————————————————————————————————————————————
        elif 'мне' in ph and 'скучно' in ph:
            return 'Попробуйте ' + choice(['погулять', 'поиграть', 'отдохнуть', 'заняться программированием'])
        # ————————————————————————————————————————————————————————————————
        elif 'ты' in ph and 'человек' in ph:
            return choice(['Нет, я бот', 'Конечно же нет', 'Нет, спасибо', 'Нетушки', 'Неа, я не человек'])
        # ————————————————————————————————————————————————————————————————
        elif ('твой' in ph and 'создатель' in ph) or ('кто' in ph and 'твой' in ph and 'создатель' in ph) or (
                'кто' in ph and 'тебя' in ph and 'создал' in ph):
            return choice(['Disorol', 'Disik', 'Dis', 'Тимур', 'Величайший программист', 'Человек']) + choice(
                [', естевственно', ', конечно', ''])
        # ————————————————————————————————————————————————————————————————
        elif 'мне' in ph and 'скучно' in ph:
            return 'Тогда попробуйте ' + choice(['пойти на прогулку', 'поработать', 'заняться программированием'])

        elif ('рандомн' in ph or 'случайн' in ph or 'любо' in ph) and ('числ' in ph or 'цифр' in ph):
            return str(randint(1, int(randint(1, 30) * '9')))
        # ————————————————————————————————————————————————————————————————
        elif ('рандомн' in ph or 'случайн' in ph or 'любо' in ph) and ('цвет' in ph or 'квадрат' in ph):
            self.dr = Drawer()
            self.dr.show()
            return choice(
                ['Держите', 'Вот', 'Вам должно понравиться', 'Ваши случайные цвета']) + ', просто кликайте по окну'
        # ————————————————————————————————————————————————————————————————
        ...
        # ————————————————————————————————————————————————————————————————
        elif ('очисти' in ph or 'почисти' in ph or 'очисть' in ph or 'почисть' in ph or 'удали' in ph) and (
                'чат' in ph or 'чата' in ph or 'историю' in ph or 'переписку' in ph or 'переписки' in ph):
            with open('chat.txt', 'w') as file:
                file.write('')
                return choice(['Готово', 'Всё', 'Держите']) + choice([', чистый чат!', ', чат очищен!'])
        # ————————————————————————————————————————————————————————————————
        else:
